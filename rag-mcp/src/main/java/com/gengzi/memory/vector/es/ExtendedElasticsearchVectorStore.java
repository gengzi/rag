//package com.gengzi.memory.vector.es;
//
...
//
///**
// * 继承 AbstractObservationVectorStore  构建自定义es 向量存储
// *
// * 构建自定义的es存储字段
// * 构建自定义es向量查询逻辑
// * 初始化es存储索引
...
//                                    .properties("user_id", p -> p.keyword(k -> k.index(true))) // 用户唯一标识，用于精确查询和聚合
//                                    .properties("type", p -> p.keyword(k -> k.index(true))) // 图片ID：精确匹配
//                                    // 2. 内容与元数据字段（text/keyword 组合：text 支持全文检索，keyword 支持聚合）
//                                    .properties("content", p -> p.text(t -> t.analyzer("standard"))) // 记忆内容原文，支持全文检索
//                                    // 3. 位置相关字段（keyword 类型：存储字符串，不做分词）
//                                    .properties("source_turn_id", p -> p.keyword(k -> k.index(true))) // 页码：不索引（仅存储）
//                                    .properties("confidence", p -> p.float_(k -> k.index(false))) //  // 置信度（0.0 ~ 1.0），由提取模型给出
//                                    // 4. 时间字段（date/keyword 类型：date 支持时间范围查询）
//                                    .properties("create_time", p -> p.date(d -> d.format("yyyy-MM-dd HH:mm:ss").index(true))) // 格式化时间
//                                    .properties("updated_time", p -> p.date(d -> d.format("yyyy-MM-dd HH:mm:ss").index(true))) // 格式化时间
