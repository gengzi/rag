...
package com.gengzi.memory.vector.opensearch;
 ...
 * @since 1.0.0
 */
public class ExtendedOpenSearchVectorStore extends AbstractObservationVectorStore implements InitializingBean {
    ...
    public static final String DEFAULT_MAPPING_EMBEDDING_TYPE_KNN_VECTOR_DIMENSION = """
            ...
            """;
    private static final Logger logger = LoggerFactory.getLogger(ExtendedOpenSearchVectorStore.class);
     ...
     */
    protected ExtendedOpenSearchVectorStore(Builder builder) {
        super(builder);
    ...
    public ExtendedOpenSearchVectorStore withSimilarityFunction(String similarityFunction) {
...
//                        : buildExactQuery(embedding, topK, similarityThreshold, filterExpression));
    ...
    public org.opensearch.client.opensearch.core.SearchRequest buildHybridQuery(
            String query,
            float[] embedding,
            int topK,
            Filter.Expression filterExpression) {
        ...
        Query filterQuery = buildFilterQuery(filterExpression); // 见下方辅助方法

        // 2. 构建 KNN 子查询（带过滤）
        Query knnSubQuery = Query.of(q -> q
                .knn(k -> k
                        .field("q_1024_vec")
                        .vector(embedding)
                        .k(topK)
        ...
        // 此处我们假设你只需要“向量 + 过滤”，所以第二个子查询用 match_all + filter
        Query matchAllSubQuery = Query.of(q -> q
                .bool(b -> b
                        .must(Query.of(m -> m.match(mt -> mt
                                .field("content")
    ...
    // ----------------------------
// 辅助方法：将你的 Filter.Expression 转为 OpenSearch Query
// ----------------------------
    private Query buildFilterQuery(Filter.Expression expr) {
        // 示例：假设 expr 可转为 query string
        // 你也可以递归构建 bool/term/terms 查询
        String queryString = getOpenSearchQueryString(expr);
        return Query.of(q -> q
    ...
    private List<Document> similaritySearchV2(org.opensearch.client.opensearch.core.SearchRequest searchRequest) {
        try {
            ...
            hits.sort((a, b) -> {
                ...
                double timeWeightB = Math.exp(-lambda * (now - createdAtB) / scaleMs);

                double finalScoreA = a.score() * timeWeightA;
                double finalScoreB = b.score() * timeWeightB;
    ...
    private Document toDocumentV2(Hit<OpenSearchExpandDocument> hit) {
        OpenSearchExpandDocument openSearchExpandDocument = hit.source();
        Document document = new Document(openSearchExpandDocument.getId(), openSearchExpandDocument.getContent(), openSearchExpandDocument.getMetadata());
    ...
    /**
     * 扩展文档模型，向量字段命名为 q_1024_vec
     */
    public record ExtendedOpenSearchDocument(
            String id,                // 文档ID
            String content,           // 记忆内容
            Map<String, Object> metadata, // 原metadata
            float[] q_1024_vec,       // 向量字段（命名为 q_1024_vec）
            String user_id,           // 用户ID
            String type,              // 记忆类型
            Float confidence,         // 置信度
            String created_time,        // 创建时间
            String updated_time         // 更新时间
    ) {
    }
    ...
    public static class Builder extends AbstractVectorStoreBuilder<Builder> {
         ...
         */
        @Override
        public ExtendedOpenSearchVectorStore build() {
            return new ExtendedOpenSearchVectorStore(this);
        }
